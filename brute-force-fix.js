...
for (const file of indicatorFiles) {
  ...
  // We want to find the <div className="space-y-4"> ... </div> that contains the map
  // and replace it entirely.
  
  // Regex to find the space-y-4 block and everything until the end of the map
  const blockRegex = /<div className="space-y-4">[\s\S]*?(?:checklistItems|auditItems)\.map[\s\S]*?<\/div>\s*\}\)\s*(\)|)\s*<\/div>/;
  
  // Wait, some files might have nested maps or other things.
  // Let's use a more targeted replacement for the map itself.
  ...
  // Find the block from <div className="space-y-4"> to the end of the map.
  // We need to be careful not to consume more than needed.
  // Usually it ends with )) } </div> or similar.
  
  let replaced = content.replace(blockRegex, newMapBlock);
  if (content === replaced) {
      // Try alternative regex if the first one fails
      const altRegex = /<div className="space-y-4">[\s\S]*?<\/div>\s*<\/div>\n\s*<LiveStatisticsCard/;
      const newMapBlockWithStats = newMapBlock + '\n\n        <LiveStatisticsCard';
      replaced = content.replace(altRegex, newMapBlockWithStats);
  }

  if (content !== replaced) {
       fs.writeFileSync(filePath, replaced, 'utf8');
       console.log("Brute force updated " + file);
  } else {
       console.log("Could not brute force " + file);
  }
}
