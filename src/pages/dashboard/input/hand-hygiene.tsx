...
import { 
  ArrowLeft, Save, CheckCircle2, Clock, User, Building2, Stethoscope, Activity, Settings, AlertCircle, RefreshCw, X, Edit2, Trash2
} from 'lucide-react';
...
export default function HandHygieneAuditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        m1: momenData.m1, m2: momenData.m2, m3: momenData.m3, m4: momenData.m4, m5: momenData.m5,
        patuh: stats.patuh, peluang: stats.peluang, persentase: stats.persentase,
      ...
      const { data: sessionData, error: sessionError } = await supabase
        ...
        .insert([{
          ...
          data_indikator: { m1: payload.m1, m2: payload.m2, m3: payload.m3, m4: payload.m4, m5: payload.m5 }
        }])
        .select('*')
        .single();
      ...
      // Backward compatibility to old table
  ...
  const handleEndTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const [hours, mins] = e.target.value.split(':').map(Number);
    const newD = endTime ? new Date(endTime) : (startTime ? new Date(startTime) : new Date());
    newD.setHours(hours, mins);
    setEndTime(newD);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto pb-32">
      ...
      <div className="flex items-center gap-6 mb-8 py-6 border-b border-white/5">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 via-blue-500 to-emerald-500 dark:from-blue-400 dark:via-purple-500 dark:to-blue-400 bg-[length:200%_auto] animate-gradient transition-all drop-shadow-sm dark:drop-shadow-[0_0_10px_rgba(59,130,246,0.4)] uppercase">Audit Kebersihan Tangan</h1>
          <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.1em] text-emerald-600 dark:text-blue-400 mt-1">Kepatuhan 5 Momen dan 6 Langkah Cuci Tangan</p>
        </div>
      </div>
      ...
      <div className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/5 backdrop-blur-xl p-8 rounded-[32px] border border-white/5 shadow-2xl">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="px-4 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-full">
                  <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest leading-none flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse" /> Durasi: {getDuration()}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="relative group overflow-hidden bg-white/5 p-6 rounded-[24px] border border-white/5 hover:border-blue-500/30 transition-all duration-500 shadow-inner">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-500/5 to-transparent rounded-bl-full pointer-events-none" />
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4 block">Jam Mulai</label>
                  <div className="relative flex items-center">
                    <input 
                      type="time" 
        ...
        <LiveStatisticsCard 
          totalDinilai={stats.peluang} totalPatuh={stats.patuh} totalTidakPatuh={stats.peluang - stats.patuh}
          persentase={stats.persentase} statusText={stats.statusText}
        />

        <button onClick={handleSubmit} disabled={isSubmitting || !observer || !unit || !profesi || stats.peluang === 0}
      ...
      <AnimatePresence>
        {isObserverModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <div onClick={() => setIsObserverModalOpen(false)} className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="relative w-full max-w-md bg-slate-900 border border-white/10 rounded-[2.5rem] p-8"
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-white flex items-center gap-2"><User className="w-5 h-5 text-blue-400" /> Kelola Observer</h3>
                <button onClick={() => setIsObserverModalOpen(false)} className="p-2 text-slate-500"><X className="w-5 h-5" /></button>
              </div>
              <div className="flex gap-2 mb-6">
                <input type="text" value={newObserverName} onChange={(e) => setNewObserverName(e.target.value)} placeholder="Nama Observer..." className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm text-white" />
                <button onClick={saveObserver} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold">{editObserverId ? 'Update' : 'Tambah'}</button>
              </div>
                ...
                {observers.map(o => (
                  <div key={o.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => { setNewObserverName(o.nama); setEditObserverId(o.id); }} className="p-2 text-slate-500 hover:text-blue-400"><Edit2 className="w-4 h-4" /></button>
