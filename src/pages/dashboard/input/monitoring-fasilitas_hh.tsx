...
import { 
  Activity, ArrowLeft, Save, CheckCircle2, Clock, User, Building2, Settings, FileText, AlertTriangle, RefreshCw, Trash2, Plus, X
} from 'lucide-react';
...
export default function FasilitasHandHygienePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!observer || !unit) { alert('Harap lengkapi field Supervisor dan Unit!'); return; }
    ...
    try {
      ...
      const uploadedUrls = await uploadImagesToSupabase(supabase, images, 'logos', 'audit'); // Bucket logic varies, logos used as example from settings
      ...
      // Fallback
      await supabase.from('monitoring_fasilitas_hand_hygiene').insert([{ ...sessionPayload, created_at: new Date().toISOString() }]);
  ...
  return (
    <div className="max-w-3xl mx-auto pb-32">
      ...
      <div className="flex items-center gap-6 py-6 border-b border-white/5">
        ...
        <div>
          <h1 className="text-2xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 via-blue-500 to-emerald-500 dark:from-blue-400 dark:via-purple-500 dark:to-blue-400 bg-[length:200%_auto] animate-gradient transition-all drop-shadow-sm dark:drop-shadow-[0_0_10px_rgba(59,130,246,0.4)] uppercase">Monitoring Fasilitas Kebersihan Tangan</h1>
          <p className="text-[10px] font-bold uppercase tracking-widest text-blue-400 mt-1">Audit ketersediaan sarana kebersihan tangan</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-8">
        <div className="bg-white/5 backdrop-blur-xl p-8 rounded-[2rem] border border-white/5 space-y-6">
          <div className="grid sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-3 sm:col-span-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block">Unit Kerja</label>
        ...
        <div className="bg-white/5 backdrop-blur-xl p-8 rounded-[2rem] border border-white/5 space-y-4">
          <h2 className="text-xs font-bold uppercase tracking-widest text-blue-400 border-b border-white/5 pb-4">Checklist Fasilitas</h2>
...
FasilitasHandHygienePage.getLayout = function getLayout(page: ReactElement) {
