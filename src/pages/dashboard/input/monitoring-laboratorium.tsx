...
const checklistItems = [
  {
    ...
    items: [
      ...
      { id: 'a6', label: 'Mengetahui cara penanganan tumpahan cairan tubuh dengan menggunakan spill kit' },
      { id: 'a7', label: 'Sudah mendapatkan imunisasi vaksin penyakit menular' },
      { id: 'a8', label: 'Melapor bila terpajan limbah infeksius (cair, tajam)' },
    ]
  },
  {
    section: 'B. FASILITAS',
    items: [
      ...
      { id: 'b2', label: 'Tersedia spill kit untuk penanganan tumpahan cairan tubuh' },
      { id: 'b3', label: 'Trolley pengangkutan sampah tertutup' },
      { id: 'b4', label: 'Trolley pengangkutan sampah dibersihkan setelah selesai digunakan' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'c5', label: 'Sampel feces dan sputum dibuang ke limbah infeksius bersama tabungnya' },
      { id: 'c6', label: 'Sampel urine dibuang ke spoelhoek, wadah dibuang ke limbah infeksius' },
    ]
  },
  {
    section: 'D. PROSES PEMBUANGAN DARAH DAN KOMPONEN DARAH',
    items: [
      { id: 'd1', label: 'Darah dan komponen darah yang sudah diambil ke ruangan dan tidak terpakai, dikembalikan ke unit pelayanan darah' },
      { id: 'd2', label: 'Darah dan komponen darah yang rusak atau kadaluwarsa dikumpulkan di tempat khusus penyimpanan darah di unit pelayanan darah' },
      { id: 'd3', label: 'Petugas kebersihan melakukan kebersihan tangan saat mengolah limbah darah dan komponen darah' },
      { id: 'd4', label: 'Petugas menggunakan APD' },
      { id: 'd5', label: 'Petugas membuang darah dan komponen darah yang rusak/kadaluwarsa ke IPAL melalui spoelhoek' },
      { id: 'd6', label: 'Ada berita acara pembuangan sisa darah rusak dan kadaluwarsa' },
      { id: 'd7', label: 'Sampel darah dibuang ke limbah infeksius bersama tabungnya' },
    ]
  },
  {
    ...
    items: [
      ...
      { id: 'e3', label: 'Pemisahan limbah benda tajam dilakukan segera oleh penghasil limbah' },
      { id: 'e4', label: 'Tidak memberikan benda tajam habis pakai ke orang lain' },
      { id: 'e5', label: 'Jika harus memberikan benda tajam ke orang lain gunakan container' },
      { id: 'e6', label: 'Limbah benda tajam dimasukkan ke safety box' },
      { id: 'e7', label: 'Safety box sesuai standar, posisi aman dan tertutup' },
      { id: 'e8', label: 'Safety box jika ¾ penuh (atau >48 jam) ditutup rapat dan dibuang' },
  ...
  },
  {
    ...
    items: [
      { id: 'f1', label: 'Isi tempat sampah berkantong kuning hanya limbah infeksius' },
      { id: 'f2', label: 'Isi tempat sampah berkantong hitam hanya limbah non infeksius' },
      { id: 'f3', label: 'Bila sudah ¾ penuh, kantong plastik diikat dan diangkut petugas' },
      { id: 'f4', label: 'Tidak dilakukan pengosongan sampah dari satu kantong ke kantong lainnya' },
      { id: 'f5', label: 'Lakukan penggantian kresek tempat sampah setelah diangkut' },
...
export default function LaboratoriumInputPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('audit_laboratorium').insert([payload]);
      if (error) throw error;
  ...
  return (
    <div className="max-w-4xl mx-auto pb-40">
      ...
      <div className="flex items-center gap-6 py-6 border-b border-white/5">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500 uppercase">Input Audit Laboratorium</h1>
          <p className="text-[11px] lg:text-xs font-bold uppercase tracking-widest text-slate-400 mt-1">Audit kepatuhan Pencegahan dan Pengendalian Infeksi area Laboratorium</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-8">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              ...
              <select value={ruangan} onChange={(e) => setRuangan(e.target.value)} required className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm text-slate-800 dark:text-white outline-none">
                <option value="Laboratorium" className="dark:bg-slate-900">Laboratorium</option>
              </select>
            </div>
            <div className="space-y-3">
        ...
        <div className="bg-white dark:bg-[#111827] shadow-sm dark:shadow-none p-6 lg:p-8 rounded-2xl border border-slate-200 dark:border-white/5 space-y-8">
          <h2 className="text-xs font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400 border-b border-slate-200 dark:border-white/5 pb-4">2. CEKLIST LABORATORIUM</h2>
        ...
        <div className="bg-white dark:bg-[#111827] shadow-sm dark:shadow-none p-6 lg:p-8 rounded-2xl border border-slate-200 dark:border-white/5 space-y-6">
          <div className="space-y-3">
            ...
            <textarea value={temuan} onChange={(e) => setTemuan(e.target.value)} placeholder="Tulis temuan audit..." className="w-full h-32 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-sm text-slate-800 dark:text-white outline-none resize-none focus:border-blue-500" />
          </div>
          <div className="space-y-3">
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block">5. REKOMENDASI</label>
            <textarea value={rekomendasi} onChange={(e) => setRekomendasi(e.target.value)} placeholder="Tulis rekomendasi perbaikan..." className="w-full h-32 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-sm text-slate-800 dark:text-white outline-none resize-none focus:border-blue-500" />
          </div>
        </div>
