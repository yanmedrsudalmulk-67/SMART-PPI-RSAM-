        ...
        const loadEditData = async () => {
          ...
          if (ed && !error) {
            ...
            if (!indicatorsData && !checklistJson && ed.indikator_id) {
               let { data: specData } = await supabase.from("audit_penatalaksanaan_linen").select("*").eq("id", id).single();
               if (!specData) {
                  const { data: specOldData } = await supabase.from("penatalaksanaan_linen").select("*").eq("id", id).single();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && editId) {
        const { error: errUpdate } = await supabase.from("audit_sessions").update(payload).eq("id", editId);
