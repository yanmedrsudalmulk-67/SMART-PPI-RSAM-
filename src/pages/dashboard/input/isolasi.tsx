      ...
      <div className="relative flex p-1.5 bg-white/80 dark:bg-[#0B1120]/80 backdrop-blur-xl rounded-full mb-8 border border-white/20 dark:border-white/10 shadow-[inset_0_1px_4px_rgba(0,0,0,0.05)]">
        <motion.div 
          className={`absolute top-1.5 bottom-1.5 rounded-full transition-colors duration-500 border ${
            activeTab === 'standar' ? 'bg-gradient-to-r from-blue-600 to-blue-500 shadow-[0_0_20px_rgba(37,99,235,0.4)] border-blue-400/30' :
            activeTab === 'transmisi' ? 'bg-gradient-to-r from-purple-600 to-purple-500 shadow-[0_0_20px_rgba(139,92,246,0.4)] border-purple-400/30' :
        ...
        {[
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 text-xs font-bold uppercase tracking-widest rounded-full transition-colors relative z-10 ${
              activeTab === tab.id ? 'text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-white'
            }`}
          >
            <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? 'text-white' : 'text-slate-500 group-hover:text-slate-700 dark:text-slate-400 dark:group-hover:text-white'}`} /> {tab.label}
          </button>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {currentIndicators.map((ind) => {
            ...
            const btnBg = isStandar ? 'bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 shadow-[0_8px_20px_rgba(37,99,235,0.3)] hover:shadow-[0_10px_25px_rgba(37,99,235,0.5)] border border-blue-400/20' :
                   isTransmisi ? 'bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-500 hover:to-purple-400 shadow-[0_8px_20px_rgba(139,92,246,0.3)] hover:shadow-[0_10px_25px_rgba(139,92,246,0.5)] border border-purple-400/20' :
            ...
            const iconColor = isStandar ? 'text-blue-600 dark:text-blue-400' : isTransmisi ? 'text-purple-600 dark:text-purple-400' : 'text-emerald-600 dark:text-emerald-400';

            const tabColors = {
              hoverBorder: isStandar ? 'hover:border-blue-500/50' : isTransmisi ? 'hover:border-purple-500/50' : 'hover:border-emerald-500/50',
              hoverText: isStandar ? 'group-hover:text-blue-600 dark:group-hover:text-blue-400' : isTransmisi ? 'group-hover:text-purple-600 dark:group-hover:text-purple-400' : 'group-hover:text-emerald-600 dark:group-hover:text-emerald-400',
              hoverShadow: isStandar ? 'hover:shadow-[0_10px_40px_-10px_rgba(37,99,235,0.15)] dark:hover:shadow-[0_10px_40px_-10px_rgba(37,99,235,0.2)]' : isTransmisi ? 'hover:shadow-[0_10px_40px_-10px_rgba(139,92,246,0.15)] dark:hover:shadow-[0_10px_40px_-10px_rgba(139,92,246,0.2)]' : 'hover:shadow-[0_10px_40px_-10px_rgba(16,185,129,0.15)] dark:hover:shadow-[0_10px_40px_-10px_rgba(16,185,129,0.2)]',
            };

            return (
            <div key={ind.id} className={`bg-white/80 dark:bg-[#0B1120]/80 backdrop-blur-xl p-6 rounded-[24px] border border-slate-200 dark:border-white/5 flex flex-col justify-between group ${tabColors.hoverBorder} ${tabColors.hoverShadow} transition-all duration-300 transform hover:-translate-y-1`}>
              <div className="space-y-4">
                <div className={`p-3.5 rounded-[18px] w-fit backdrop-blur-sm group-hover:scale-110 transition-transform duration-300 ${iconBoxColor}`}>
                  <ind.icon className={`w-6 h-6 ${iconColor}`} />
                </div>
                <div>
                  <h3 className={`font-bold text-slate-800 dark:text-slate-100 text-[17px] ${tabColors.hoverText} transition-colors leading-tight`}>{ind.title}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 line-clamp-3">{ind.desc}</p>
                </div>
              </div>
                ...
                >
                  ...
                  <div className="relative flex items-center gap-3">
                    ...
                    <span className="text-white !text-white group-hover/btn:tracking-[0.2em] transition-all">INPUT DATA</span>
                  </div>
                </Link>
            </div>
            );
