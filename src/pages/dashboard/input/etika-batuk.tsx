...
const etikaItems = [
  { id: 'masker', label: '1. Ketersediaan Masker / Tisu', key: 'masker' },
  { id: 'tempat_sampah', label: '2. Ketersediaan Tempat Sampah Infeksius', key: 'tempat_sampah' },
  { id: 'edukasi', label: '3. Edukasi diberikan (Lisan / Poster)', key: 'edukasi' },
  { id: 'kepatuhan', label: '4. Pasien / Pengunjung mematuhi etika batuk', key: 'kepatuhan' }
] as const;

type EtikaStatus = 'ya' | 'tidak' | 'na' | null;
...
export default function InputEtikaBatukPage() {
  ...
  const [etikaData, setEtikaData] = useState<Record<string, EtikaStatus>>({
    masker: null, tempat_sampah: null, edukasi: null, kepatuhan: null
  });
  ...
  const handleActionClick = (id: string, stat: EtikaStatus) => {
    setEtikaData(prev => ({ ...prev, [id]: stat }));
  };
  ...
  const stats = useMemo(() => {
    ...
    Object.values(etikaData).forEach(val => {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-32">
      <AnimatePresence>
        {showToast && (
          ...
          >
            ...
            Data Audit Etika Batuk Tersimpan!
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center gap-6 mb-8 py-6 border-b border-white/5">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 via-blue-500 to-emerald-500 dark:from-blue-400 dark:via-purple-500 dark:to-blue-400 bg-[length:200%_auto] animate-gradient transition-all drop-shadow-sm dark:drop-shadow-[0_0_10px_rgba(59,130,246,0.4)] uppercase">Audit Etika Batuk</h1>
          <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.1em] text-emerald-600 dark:text-blue-400 mt-1">Observasi kepatuhan etika batuk dan bersin</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <LiveStatisticsCard 
          ...
          persentase={stats.persentase} statusText={stats.statusText} title="KEPATUHAN ETIKA BATUK"
        />
