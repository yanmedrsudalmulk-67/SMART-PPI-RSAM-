...
export default function RadiologiInputPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('audit_radiologi').insert([payload]);
      if (error) throw error;
  ...
  return (
    <div className="max-w-4xl mx-auto pb-40">
      ...
      <div className="flex items-center gap-6 py-6 border-b border-white/5">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500 uppercase">Audit Radiologi</h1>
          <p className="text-[11px] lg:text-xs font-bold uppercase tracking-widest text-slate-400 mt-1">Monitoring Pencegahan dan Pengendalian Infeksi Area Radiologi</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-8">
        ...
        <div className="bg-white dark:bg-[#111827] shadow-sm dark:shadow-none p-6 lg:p-8 rounded-2xl border border-slate-200 dark:border-white/5 space-y-6">
          <div className="space-y-3">
            ...
            <textarea value={temuan} onChange={(e) => setTemuan(e.target.value)} placeholder="Contoh:&#10;Handrub kosong&#10;Tidak ada spill kit&#10;Lead apron belum dibersihkan" className="w-full h-32 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-sm text-slate-800 dark:text-white outline-none resize-none focus:border-blue-500" />
          </div>
          <div className="space-y-3">
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block">5. REKOMENDASI</label>
            <textarea value={rekomendasi} onChange={(e) => setRekomendasi(e.target.value)} placeholder="Contoh:&#10;Lengkapi spill kit&#10;Jadwalkan disinfeksi rutin&#10;Ganti handrub baru" className="w-full h-32 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-sm text-slate-800 dark:text-white outline-none resize-none focus:border-blue-500" />
          </div>
        </div>
