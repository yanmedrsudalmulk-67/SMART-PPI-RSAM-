...
const checklistItems = genericAuditConfigs.monitoring_gizi?.items || [];
const tableName = genericAuditConfigs.monitoring_gizi?.tableName || 'audit_monitoring_gizi';
...
export default function MonitoringGiziPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase.from(tableName || 'audit_gizi').insert([{ ...sessionPayload, created_at: new Date().toISOString() }]);
  ...
  return (
    <div className="max-w-3xl mx-auto pb-32">
      ...
      <div className="flex items-center gap-6 py-6 border-b border-white/5">
        ...
        <div>
          <h1 className="text-2xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 via-blue-500 to-emerald-500 dark:from-blue-400 dark:via-purple-500 dark:to-blue-400 bg-[length:200%_auto] animate-gradient transition-all drop-shadow-sm dark:drop-shadow-[0_0_10px_rgba(59,130,246,0.4)] uppercase">Monitoring Gizi</h1>
          <p className="text-[10px] font-bold uppercase tracking-widest text-blue-400 mt-1">Audit higiene sanitasi makanan dan dapur gizi</p>
        </div>
      </div>
