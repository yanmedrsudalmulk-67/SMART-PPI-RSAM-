...
export default function JenazahReport({ filters }: { filters?: any }) {
  const [data, setData] = useState<JenazahRecord[]>([]);
  ...
  const { trendData, summaryStats, subsectionCompliance, commonFindings } = useMemo(() => {
    ...
    const subsectionMap: Record<string, { patuh: number; total: number }> = {
      'KEBERSIHAN RUANGAN DAN PERALATAN': { patuh: 0, total: 0 },
      'FASILITAS': { patuh: 0, total: 0 }
    };
  ...
  if (loading) return <div className="p-20 text-center text-slate-900 dark:text-white">Memuat Data Kamar Jenazah...</div>;
  if (data.length === 0) return <div className="p-12 text-center text-slate-500 font-bold uppercase">Belum ada data audit Kamar Jenazah.</div>;
              ...
              {selectedRecord && (
                  <OfficialReportSheet 
                      data={{
                        ...
                        observer: selectedRecord.supervisor, // map supervisor to observer for generic report 
