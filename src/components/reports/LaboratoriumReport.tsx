...
export default function LaboratoriumReport({ filters }: { filters?: any }) {
  const [data, setData] = useState<LaboratoriumRecord[]>([]);
  ...
  const { trendData, summaryStats, subsectionCompliance, commonFindings } = useMemo(() => {
    ...
    const subsectionMap: Record<string, { patuh: number; total: number }> = {
      'A. PERSONAL': { patuh: 0, total: 0 },
      'B. FASILITAS': { patuh: 0, total: 0 },
      'C. PROSES PENGELOLAAN LIMBAH CAIR (INFEKSIUS)': { patuh: 0, total: 0 },
      'D. PROSES PEMBUANGAN DARAH DAN KOMPONEN DARAH': { patuh: 0, total: 0 },
      'E. PROSES PENGELOLAAN LIMBAH BENDA TAJAM': { patuh: 0, total: 0 },
      'F. PROSES PENGELOLAAN LIMBAH PADAT': { patuh: 0, total: 0 }
    };
  ...
  if (loading) return <div className="p-20 text-center text-slate-900 dark:text-white">Memuat Data Laboratorium...</div>;
  if (data.length === 0) return <div className="p-12 text-center text-slate-500 font-bold uppercase">Belum ada data audit Laboratorium.</div>;
