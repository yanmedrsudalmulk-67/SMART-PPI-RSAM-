...
for (const filePath of files) {
  ...
  // Strip `handleFileUpload` function
  // It handles async function block removal
  content = content.replace(/const handleFileUpload = async.*?\}?;\s*const removeImage = .*?;\s*\};\s/s, '');
  content = content.replace(/const handleFileUpload = async[\s\S]*?(?=const handleSubmit = async)/g, '');

  
  // Strip duplicate upload inside `handleSubmit`
  const uploadBlock = /let finalDokumentasiUrls: string\[\] = \[\];\s*\/\/\s*Upload Gambar(?:[\s\S]*?)(?=const ttdPj|const payload)/;
