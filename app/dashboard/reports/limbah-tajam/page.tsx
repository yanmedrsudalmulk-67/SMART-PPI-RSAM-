...
export default function LaporanLimbahTajam() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const supabase = getSupabase();
      const { error } = await supabase.from('audit_pengelolaan_limbah_tajam').delete().eq('id', id);
      if (error) throw error;
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-heading font-bold text-gradient text-indigo-400">Laporan Pengelolaan Limbah Tajam</h1>
            <p className="text-xs text-slate-500">Arsip data kepatuhan pengelolaan limbah tajam</p>
          </div>
        </div>
        ...
        <div className="glass-card p-4 rounded-2xl border-white/5 bg-navy-light/30">
          <div className="relative">
            ...
            >
              ...
              <option>Ranap Khadijah</option>
              <option>Ranap Usman</option>
            </select>
          </div>
        </div>
      </div>
