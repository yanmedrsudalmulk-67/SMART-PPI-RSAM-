...
interface AuditPetugas {
  id: string;
  tanggal_waktu: string;
  observer: string;
  unit: string;
  data_indikator: Record<string, string>;
  created_at: string;
}

export default function PerlindunganPetugasReportPage() {
  const [data, setData] = useState<AuditPetugas[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterUnit, setFilterUnit] = useState('All');
  const [selectedAudit, setSelectedAudit] = useState<AuditPetugas | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error fetching health protection audit data:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-navy-dark/50 backdrop-blur-md p-6 rounded-[2rem] border border-white/5 shadow-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-heading font-bold text-white tracking-tight">Data Audit Perlindungan Kesehatan Petugas</h1>
            <p className="text-xs text-blue-400 font-bold uppercase tracking-widest mt-1">Histori laporan pemeriksaan & imunisasi petugas</p>
          </div>
        </div>
              ...
              {loading ? (
                ...
                filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-white/[0.03] transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <p className="text-sm font-bold text-white group-hover:text-blue-400 transition-colors">{item.observer}</p>
                    </td>
                    <td className="px-6 py-5">
        ...
        {selectedAudit && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl max-h-[90vh] bg-navy-light border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col"
            >
              {/* Modal Header */}
              <div className="p-6 sm:p-8 border-b border-white/5 bg-white/[0.02] flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                     <h2 className="text-xl font-heading font-bold text-white tracking-tight leading-tight">Detail Audit Perlindungan Petugas</h2>
                ...
                <button onClick={() => setSelectedAudit(null)} className="p-3 bg-white/5 hover:bg-white/10 rounded-2xl text-slate-500 hover:text-white transition-all active:scale-90">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="p-6 sm:p-8 overflow-y-auto custom-scrollbar space-y-8">
                <div className="grid grid-cols-2 gap-4">
                   <div className="bg-white/5 rounded-2xl p-6 border border-white/5 text-center shadow-inner">
                      ...
                      <p className="text-sm font-bold text-white truncate">{selectedAudit.observer}</p>
                   </div>
                   <div className="bg-white/5 rounded-2xl p-6 border border-white/5 text-center shadow-inner">
                ...
                <div className="space-y-4 pt-4">
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-400 border-l-2 border-blue-600 pl-3">Checklist Perlindungan Kesehatan Petugas</h3>
                  <div className="space-y-3">
                    {Object.entries(selectedAudit.data_indikator).map(([id, status]) => (
                      <div key={id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 group hover:bg-white/[0.08] transition-all">
                        <p className="text-xs font-medium text-slate-300 max-w-[70%] leading-relaxed">
                          {id === 'item_1' ? 'Imunisasi (Hepatitis B & Covid-19)' : 'Pemeriksaan Kesehatan Berkala'}
                        </p>
                        <span className={`px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-sm ${
                          ...
                          {status === 'ya' ? 'Sudah' : status === 'tidak' ? 'Belum' : 'N/A'}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="pt-8 flex flex-col items-center justify-center text-slate-600 text-[10px] font-bold uppercase tracking-widest gap-2">
