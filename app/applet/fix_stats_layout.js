...
const updateStatsToDekontaminasi = () => {
    files.forEach(f => {
        ...
        content = content.replace(/className="glass-card [^"]*?flex flex-col items-center justify-center relative overflow-hidden"/g, 
            'className="glass-card p-6 sm:p-8 rounded-[32px] border-white/5 flex flex-col md:flex-row items-center justify-center gap-8 relative overflow-hidden"');
        ...
        */
        ...
        // Let's also fix the third box in monitoring layouts (Status box) which takes col-span-2:
        content = content.replace(/<div className={`col-span-2 p-4 rounded-2xl border \${stats.color} \${stats.bg} border-current\/20 flex items-center justify-center gap-2`}>\s*<p className="text-sm font-bold uppercase tracking-widest">Status: \{stats\.status\}<\/p>\s*<\/div>/g, ""); // Remove it altogether, the status is already under the percentage!
